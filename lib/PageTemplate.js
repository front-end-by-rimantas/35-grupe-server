...
class PageTemplate {
    ...
    headerHTML() {
        ...
        if (this.data.user.isLoggedIn) {
            navHTML = `<a href="/account" class="${path === 'account' ? 'active' : ''}">Account</a>
                    <a id="logout" href="/logout" class="${path === 'logout' ? 'active' : ''}">Logout</a>`;
        } else {
    ...
    scriptHTML() {
        let HTML = `<script src="/js/pages/_common.js" type="module" defer></script>`;
        if (this.jsForPage !== '') {
            HTML += `<script src="/js/pages/${this.jsForPage}.js" type="module" defer></script>`;
        }
        return HTML;
    }
