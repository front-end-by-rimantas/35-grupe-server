...
server.httpServer = http.createServer(async (req, res) => {
    ...
    req.on('end', async () => {
        ...
        const [isLoggedIn, sessionObj] = await APItoken._innerMethods.verify(dataForHandlers.cookies['server-35-token']);
        if (isLoggedIn) {
            dataForHandlers.user.isLoggedIn = true;
            dataForHandlers.user.session = sessionObj;
        }
        ...
        if (isPage) {
            ...
            const [HTML, headers] = await pageObj.render();
